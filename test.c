...
int main(int argc, char *argv[]) {
    ...
    while ((bytes_read = read(fd, &current_char, 1)) == 1) {
        ...
        if (buffer_idx < sizeof(num_str_buffer) -1 && isdigit(current_char)) {
            ...
            perror("Error en el formato del archivo.");
            close(fd);
            exit(EXIT_FAILURE);
        }
    }
    ...
    for (int i = 0; i < total_cola; i++) {
        if ((bytes_read = read(fd, &current_char, 1)) == 1) {
            if (current_char == '0') {
                clientes[i].is_vip = 0;
            ...
            perror("Error leyendo los clientes.");
            free(clientes);
            free(hilos);
            close(fd);
        }
        read(fd, &current_char, 1); // leer salto de linea
